...
interface DashboardMetrics {
  ...
  activeJobs: DashboardMetric;
  outstandingInvoices: DashboardMetric;
  materialInventory: DashboardMetric;
  sopCompliance: DashboardMetric;
  teamTraining: DashboardMetric;
...
router.get('/', async (req: Request, res: Response) => {
  try {
    // Fetch real data from database where available
    ...
    const metrics: DashboardMetrics = {
      ...
      activeJobs: {
        value: projectCount || 12,
        change: 3,
        positive: true,
        formattedValue: String(projectCount || 12),
...
// GET /api/metrics/sop-stats - Get SOP statistics
router.get('/sop-stats', async (req: Request, res: Response) => {
  try {
    const [totalProcedures, activeProcedures, categories] = await Promise.all([
