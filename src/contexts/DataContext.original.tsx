...
export const DataProvider: React.FC<DataProviderProps> = ({ children }) => {
  ...
  const [trainingLoading, setTrainingLoading] = useState<boolean>(true);
  ...
  const addEmployee = async (employee: Employee) => {
    console.log('👤 DataStore: Adding employee via API:', employee.firstName, employee.lastName);
    setEmployeesError(null);

    try {
      const response = await EmployeeService.create(employee);
      if (response.error) {
        setEmployeesError(response.error);
      ...
      console.log('✅ Employee added successfully:', response.data.firstName, response.data.lastName);
    } catch (error) {
  ...
  const updateEmployee = async (id: string, updates: Partial<Employee>) => {
    console.log('👤 DataStore: Updating employee via API:', id);
    setEmployeesError(null);

    try {
      const response = await EmployeeService.update(id, updates);
      if (response.error) {
        setEmployeesError(response.error);
      ...
      console.log('✅ Employee updated successfully:', response.data.firstName, response.data.lastName);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      setEmployeesError(errorMessage);
  ...
  const deleteEmployee = async (id: string) => {
    console.log('👤 DataStore: Deleting employee via API:', id);
    setEmployeesError(null);
  ...
  const addTrainingSession = async (session: TrainingSession) => {
    console.log('📚 DataStore: Adding training session via API:', session.type, 'for', session.employeeName);
    setTrainingError(null);

    try {
      const response = await TrainingService.create(session);
      if (response.error) {
        setTrainingError(response.error);
        console.error('Failed to add training session:', response.error);
        return;
      }

      // Add to local state
      setTrainingSessions(prev => [...prev, response.data]);
      console.log('✅ Training session added successfully:', response.data.type);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
  ...
  const updateTrainingSession = async (id: string, updates: Partial<TrainingSession>) => {
    console.log('📚 DataStore: Updating training session via API:', id);
    setTrainingError(null);

    try {
      const response = await TrainingService.update(id, updates);
      if (response.error) {
        setTrainingError(response.error);
        console.error('Failed to update training session:', response.error);
        return;
      }
      ...
      console.log('✅ Training session updated successfully:', response.data.type);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      setTrainingError(errorMessage);
  ...
  const deleteTrainingSession = async (id: string) => {
    console.log('📚 DataStore: Deleting training session via API:', id);
    setTrainingError(null);
    ...
    try {
      ...
      if (response.error) {
        setTrainingError(response.error);
        console.error('Failed to delete training session:', response.error);
        return;
      }
  ...
  const getUpcomingTrainingSessions = () => {
    const now = new Date();
  ...
  useEffect(() => {
    const loadEmployees = async () => {
      ...
      try {
        ...
        if (response.error) {
          console.warn('Failed to load employees from API, using fallback:', response.error);
          setEmployeesError(response.error);
          setEmployees(sampleEmployees); // Fallback to sample data
        } else {
          console.log('✅ Employees loaded from API:', response.data?.length || 0);
          setEmployees(response.data || []);
        }
      } catch (error) {
        console.warn('Failed to load employees from API:', error);
        setEmployeesError(error instanceof Error ? error.message : 'Unknown error');
        setEmployees(sampleEmployees); // Fallback to sample data
      } finally {
  ...
  // Load training sessions from API on component mount
  useEffect(() => {
    const loadTrainingSessions = async () => {
      ...
      try {
        console.log('📚 Loading training sessions from API...');
        const response = await TrainingService.getAll();

        if (response.error) {
          console.warn('Failed to load training sessions from API:', response.error);
          setTrainingError(response.error);
          setTrainingSessions([]); // Empty array as fallback
        } else {
          console.log('✅ Training sessions loaded from API:', response.data?.length || 0);
          setTrainingSessions(response.data || []);
        }
      } catch (error) {
        console.warn('Failed to load training sessions from API:', error);
        setTrainingError(error instanceof Error ? error.message : 'Unknown error');
        setTrainingSessions([]);
      } finally {
...
export type { Invoice, Transaction, Job, JournalEntry, JournalEntryLine, Account, Lead, Opportunity, Employee, Certification, TrainingSession };
